...
/*   Updated: 2019/07/01 16:51:46 by viclucas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "malloc.h"

void		print_page_hexa(int type, void *addr)
{
	if (type == 1)
		ft_putstr("TINY : ");
	else if (type == 2)
...
void		print_block_hexa(void *addr, size_t size)
{
	ft_putstr("0x");
	hexa_for((size_t)addr);
	ft_putstr(" - 0x");
...
size_t		print_content_hexa(t_page *tmp)
{
	size_t			count;
	t_block			*tmp_block;
